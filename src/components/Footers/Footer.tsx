...
const Footer = () => {
	...
	return (
    <footer className="bg-[#F7F7F7] w-full py-2 flex flex-col item-center mb-[-100px]">
      <div className="mx-auto max-w-[1400px] w-full hidden slg:block">
        <section className="flex justify-center gap-16 mt-2">
          <div className="flex flex-col gap-4 w-[80%]">
            <LogoImage className="!w-[150px] h-10 cursor-pointer brightness-200" />
              ...
              <div key={index} className="flex flex-col gap-4 lg:gap-5 w-full">
                ...
                {section.links.map((link, linkIndex) => (
                  <Link
                    ...
                    className="text-black/80 text-sm leading-[1.3] hover:text-[#EAF586] transition-[.3]"
                  >
                    {link.label}
                  </Link>
                ))}
      ...
      <div className="mx-auto flex w-full flex-col slg:hidden mb-4">
        <section className="flex flex-col justify-between gap-1 sm:gap-6 mt-2 px-2 xs:px-6 sm:px-10">
          <div className="flex w-full justify-between items-end gap-4">
            <div className="">
              <LogoImage className="!w-[100px] h-10 cursor-pointer brightness-200" />
            </div>
              ...
              <div key={index} className="flex flex-col gap-2 sm:gap-5 w-full">
                ...
                {section.links.map((link, linkIndex) => (
                  <Link
                    key={linkIndex}
                    href={link.href}
                    className={`${link.href === pathname ? "text-[#EAF586]" : "text-black"} text-xs sm:text-sm font-[400] hover:text-[#EAF586] transition-[.3] leading-6`}
                  >
                    {link.label}
                  </Link>
                ))}
